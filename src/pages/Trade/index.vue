<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="(item, index) in addressInfo" :key="item.id">
        <span :class="{ selected: item.isDefault == '1' }" class="username">{{
          item.consignee
        }}</span>
        <p @click="changeDefault(item)">
    ...
    <div class="trade">
      ...
      <div class="receiveInfo">
        寄送至:
        <span>{{ curDefaultAdress.userAddress }}</span>
        收货人：<span>{{ curDefaultAdress.consignee }}</span>
        <span>{{ curDefaultAdress.phoneNum }}</span>
      </div>
    </div>
  ...
  export default {
    ...
    methods: {
      ...
      async submitOrder() {
        ...
        let data = {
          consignee: this.curDefaultAdress.consignee, //最终收件人的名字
          consigneeTel: this.curDefaultAdress.phoneNum, //最终收件人的手机号
          deliveryAddress: this.curDefaultAdress.fullAddress, //收件人的地址
