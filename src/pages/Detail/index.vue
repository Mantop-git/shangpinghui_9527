    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          <!-- 可以等skuInfo数据获取完成后在渲染，也可以设置计算属性给skuInfo || [] 默认值 -->
          <Zoom :skuImageList="skuInfo.skuImageList" v-if="skuInfo.skuImageList" />
            ...
            <div class="chooseArea">
              ...
              <dl v-for="(spuSaleAttr, index1) in spuSaleAttrList" :key="spuSaleAttr.id">
                ...
                <dd
                  ...
                  @click="
                    changeActive(spuSaleAttrValue, spuSaleAttr.spuSaleAttrValueList)
                  "
                >
  ...
  export default {
    ...
    methods: {
      ...
      changeActive(spuSaleAttr, spuSaleAttrValue) {
        //对spuSaleAttrValue进行遍历把里面控制高亮的属性全变无
        //spuSaleAttr为我们点击的那个属性，在把ta的值高亮
        spuSaleAttrValue.map(item => {
          item.isChecked = 0;
          return item;
        });
        spuSaleAttr.isChecked = 1;
        console.log(spuSaleAttr, "a");
        console.log(spuSaleAttrValue, "b");
      },
      //输入框
      changeSkunum() {
        if (this.skuNum < 1) this.skuNum = 1; //输入值小于1
        if (isNaN(this.skuNum)) this.skuNum = 1; //输入值不是数字
        this.skuNum = parseInt(this.skuNum); //输入的值不是整数
